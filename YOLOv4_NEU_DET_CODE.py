# YOLOv4_NEU_DET -- Code extracted from notebook
...
# CODE CELL 13
# ======================================================================
print("YOLOv4 will use: darknet_train() / darknet_evaluate() / darknet_inference()")
print("Binary builder: build_modern_darknet")
print("Weights will be saved under:", MODEL_DIR["YOLOv4"]["weights"])


# ======================================================================
# CODE CELL 14
# ======================================================================
_bin, _obj_data, _cfg, _train_info = darknet_train(
    version_tag="v4", model_name="YOLOv4", cfg_filename="yolov4.cfg",
    layer_type="yolo", patch_fn=lambda t,n,batch,width,height,max_batches: patch_cfg_yolo_or_region(t,n,batch,width,height,max_batches,'[yolo]'),
    pretrained_url="https://github.com/AlexeyAB/darknet/releases/download/darknet_yolo_v3_optimal/yolov4.conv.137", pretrained_name="yolov4.conv.137",
    darknet_bin_getter=build_modern_darknet,
)

if _train_info is None:
    report_failure("YOLOv4", "Darknet build or cfg preparation failed -- see log above.")
    YOLOv4_ready = False
else:
    _backup_dir, _training_time_s, _train_log = _train_info
    print(f"\nTraining time: {_training_time_s:.1f} s")
    YOLOv4_ready = True
...
# ======================================================================
if YOLOv4_ready:
    _eval = darknet_evaluate(_bin, _obj_data, _cfg, _backup_dir, _training_time_s)
    if _eval is None:
        report_failure("YOLOv4", "No best.weights found after training -- training may not have completed.")
        YOLOv4_ready = False
    else:
        YOLOv4_overall, YOLOv4_per_class, YOLOv4_weights = _eval
        YOLOv4_overall.update({"Architecture": "CSPDarknet53 backbone, SPP block, PANet neck, 3-scale [yolo] detection heads", "Backbone": "CSPDarknet53 (Cross-Stage-Partial connections)",
                                       "Pretrained_Weights": "yolov4.conv.137"})
        for row in YOLOv4_per_class: row["Model"] = "YOLOv4"
        save_overall_result("YOLOv4", YOLOv4_overall)
        save_per_class_result("YOLOv4", YOLOv4_per_class)
        print(YOLOv4_overall)
else:
    print("Skipping evaluation -- training did not complete.")
...
# ======================================================================
if YOLOv4_ready:
    _iters, _losses, _map_vals = darknet_parse_training_log(_train_log)
    graphs_dir = MODEL_DIR["YOLOv4"]["graphs"]
    if _iters:
        plt.figure(figsize=(8,4))
        plt.plot(_iters, _losses)
        plt.xlabel("Iteration"); plt.ylabel("Avg loss"); plt.title("YOLOv4: training loss vs iteration")
        plt.grid(alpha=0.3); plt.tight_layout()
        plt.savefig(os.path.join(graphs_dir, "loss_vs_iteration.png")); plt.show()
    else:
        print("Could not parse a loss curve from this run's console log.")
    if _map_vals:
        plt.figure(figsize=(8,4))
        plt.plot(_map_vals, marker="o")
        plt.xlabel("mAP checkpoint #"); plt.ylabel("mAP"); plt.title("YOLOv4: mAP during training")
        plt.grid(alpha=0.3); plt.tight_layout()
        plt.savefig(os.path.join(graphs_dir, "map_during_training.png")); plt.show()
    else:
        print("No periodic mAP values were parsed from this run's console log.")
    print("Confusion matrix: NOT AVAILABLE -- Darknet's detector map tool doesn't generate one.")
    print("PR curve: NOT AVAILABLE -- same reason.")
else:
    print("Skipping -- training did not complete.")
...
if YOLOv4_ready:
    ...
    print("Upload an image to test with YOLOv4:")
    uploaded = files.upload()
    for fname in uploaded.keys():
        img_path = os.path.join("/content", fname)
        darknet_inference(_bin, _obj_data, _cfg, YOLOv4_weights, img_path,
                           MODEL_DIR["YOLOv4"]["inference_results"], "YOLOv4")
else:
    print("Skipping -- YOLOv4 did not train successfully.")
