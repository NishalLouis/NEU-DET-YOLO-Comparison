# YOLOv7_NEU_DET -- Code extracted from notebook
...
# CODE CELL 13
# ======================================================================
yolov7_repo_dir, yolov7_yaml_path = yolov7_setup("YOLOv7")
print("YOLOv7 repo ready at:", yolov7_repo_dir)


# ======================================================================
# CODE CELL 14
# ======================================================================
yolov7_run_dir, YOLOv7_training_time_s, _rc = yolov7_train("YOLOv7", yolov7_repo_dir, yolov7_yaml_path)
YOLOv7_ready = (_rc == 0)
if not YOLOv7_ready:
    print("\nYOLOv7 training exited with a non-zero code -- most likely the")
    print("torch/numpy dependency friction described in Cell", {c}, ".")
print(f"\nTraining time: {YOLOv7_training_time_s:.1f} s")
...
# ======================================================================
if YOLOv7_ready:
    _eval = yolov7_evaluate(yolov7_repo_dir, yolov7_yaml_path, yolov7_run_dir, YOLOv7_training_time_s)
    if _eval[0] is None:
        report_failure("YOLOv7", "No best.pt found -- training may not have completed.")
        YOLOv7_ready = False
    else:
        YOLOv7_overall, YOLOv7_weights = _eval
        YOLOv7_overall.update({"Architecture": "E-ELAN backbone with auxiliary training heads",
                                "Backbone": "E-ELAN"})
        YOLOv7_per_class = [{"Model": "YOLOv7", "Class": cls, "Precision": YOLOv7_overall["Precision"],
                              "Recall": YOLOv7_overall["Recall"], "F1": YOLOv7_overall["F1"],
                              "AP50": YOLOv7_overall.get(f"AP_{cls}")} for cls in CLASS_NAMES]
        save_overall_result("YOLOv7", YOLOv7_overall)
        save_per_class_result("YOLOv7", YOLOv7_per_class)
        print(YOLOv7_overall)
else:
...
# ======================================================================
if YOLOv7_ready:
    yolov7_curves("YOLOv7", yolov7_repo_dir)
    yolov7_confusion_pr("YOLOv7", yolov7_repo_dir)
else:
...
if YOLOv7_ready:
    ...
    print("Upload an image to test with YOLOv7:")
    uploaded = files.upload()
    for fname in uploaded.keys():
        img_path = os.path.join("/content", fname)
        yolov7_inference(yolov7_repo_dir, YOLOv7_weights, img_path,
                          MODEL_DIR["YOLOv7"]["inference_results"], "YOLOv7")
else:
    print("Skipping -- YOLOv7 did not train successfully.")
