# YOLOv6_NEU_DET -- Code extracted from notebook
...
# CODE CELL 13
# ======================================================================
yolov6_repo_dir, yolov6_yaml_path = yolov6_setup("YOLOv6")
print("YOLOv6 repo ready at:", yolov6_repo_dir)


# ======================================================================
# CODE CELL 14
# ======================================================================
yolov6_run_dir, YOLOv6_training_time_s, _rc = yolov6_train("YOLOv6", yolov6_repo_dir, yolov6_yaml_path)
YOLOv6_ready = (_rc == 0)
if not YOLOv6_ready:
    print("\nYOLOv6 training exited with a non-zero code -- check the log above.")
    print("Common cause: torch/numpy version conflicts. Restart the runtime and")
    print("re-run from Cell 1 in a clean session if this persists.")
print(f"\nTraining time: {YOLOv6_training_time_s:.1f} s")
...
# ======================================================================
if YOLOv6_ready:
    _eval = yolov6_evaluate(yolov6_repo_dir, yolov6_yaml_path, yolov6_run_dir, YOLOv6_training_time_s)
    if _eval[0] is None:
        report_failure("YOLOv6", "No best_ckpt.pt found -- training may not have completed.")
        YOLOv6_ready = False
    else:
        YOLOv6_overall, YOLOv6_weights = _eval
        YOLOv6_overall.update({"Architecture": "RepVGG-style reparameterized backbone, decoupled head",
                                "Backbone": "EfficientRep"})
        YOLOv6_per_class = [{"Model": "YOLOv6", "Class": cls, "Precision": YOLOv6_overall["Precision"],
                              "Recall": YOLOv6_overall["Recall"], "F1": YOLOv6_overall["F1"],
                              "AP50": YOLOv6_overall.get(f"AP_{cls}")} for cls in CLASS_NAMES]
        save_overall_result("YOLOv6", YOLOv6_overall)
        save_per_class_result("YOLOv6", YOLOv6_per_class)
        print(YOLOv6_overall)
else:
...
# ======================================================================
if YOLOv6_ready:
    yolov6_curves("YOLOv6", yolov6_run_dir)
else:
...
if YOLOv6_ready:
    ...
    print("Upload an image to test with YOLOv6:")
    uploaded = files.upload()
    for fname in uploaded.keys():
        img_path = os.path.join("/content", fname)
        yolov6_inference(yolov6_repo_dir, YOLOv6_weights, img_path,
                          MODEL_DIR["YOLOv6"]["inference_results"], "YOLOv6")
else:
    print("Skipping -- YOLOv6 did not train successfully.")
