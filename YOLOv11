# YOLOv11 on NEU-DET -- Industrial Surface Defect Detection

**This notebook is independent** -- it contains the full standardized dataset
pipeline (Cells 1-9), the shared helper-function library (Cell 9B -- the same
one used by every version in this series, so functions for frameworks other than
YOLOv11's are simply unused here, not a mistake), and then only YOLOv11's own
setup/train/evaluate/curves/inference cells.

Results are written to `/content/drive/MyDrive/YOLO_NEU_DET_Experiments/` in the
same layout every other version in this series uses (`YOLOv11/weights`,
`YOLOv11/results`, `YOLOv11/graphs`, `YOLOv11/inference_results`,
`YOLOv11/metrics`), plus a shared `Results/all_models_results.csv` row -- so if
you run several of these notebooks (in separate sessions, per the framework-group
guidance below), their results accumulate into one comparable table without
overwriting each other.

**Framework note:** see YOLOv11's own setup cell below for which codebase this
version actually trains through, and whether it needs a separate/isolated
runtime session from other versions to avoid dependency conflicts.

# YOLOv1 -> YOLOv11 on NEU-DET: Full Research Comparison

A single notebook training and evaluating all 11 YOLO versions on the same
NEU-DET dataset, with per-model training curves, evaluation plots, single-image
inference, and a final automatic cross-model comparison.

## The pipeline

```
NEU-DET XML dataset (Drive, read-only)
        |
Inspect + validate dataset (Cells 4-6)
        |
Convert XML -> ONE standardized YOLO dataset, reused by all 11 models (Cells 7-9)
        |
For each of 11 versions:
   Setup -> Train (50 epochs) -> Evaluate -> Curves/plots -> Upload-and-test
        |
Final comparison table + graphs + best-model selection (Cells 65-68)
        |
Upload one image, run it through every trained model side by side (Cells 69-70)
```

## Why this isn't 11 copies of the same code

YOLOv1-v11 span **four different codebases**, not one:

| Group | Versions | Framework |
|---|---|---|
| Darknet (C/CUDA) | v1, v2, v4 | `hank-ai/darknet` (v2, v4) / `pjreddie/darknet` fallback (v1 only) |
| `ultralytics` package (PyTorch, one shared API) | v3, v5, v8, v9, v10, v11 | `pip install ultralytics` |
| Meituan's own repo | v6 | `meituan/YOLOv6` |
| Original authors' repo (inference-only in Ultralytics) | v7 | `WongKinYiu/yolov7` |

**Restart the runtime between these four groups** to avoid dependency conflicts.
Every cell reloads its state from Drive, so this is always safe. Reusable
functions (`darknet_*`, `ultralytics_*`, `yolov6_*`, `yolov7_*`) are defined once
in Cell 9B and called by every version in that family, rather than duplicated
11 times.

## Two things worth knowing before you train anything

**YOLOv1's `[detection]` layer was dropped from the modern Darknet fork** --
confirmed from an actual build log, not assumed. v1's setup cell automatically
falls back to the original `pjreddie/darknet` codebase, patched for your GPU, and
trains without cuDNN acceleration (a real, documented deviation, not a silent
substitution). v2 does not have this problem -- its `[region]` layer is present in
the modern fork.

**Confusion matrices and PR curves are not available for v1/v2/v4 (Darknet).**
Classic Darknet's `detector map` tool doesn't generate these. Those cells state
this plainly rather than approximating something Darknet didn't actually produce.

## CELL 1 -- Install common dependencies

Installs only what every cell needs regardless of which YOLO version is running.
Framework-specific installs (Darknet's build tools, YOLOv6's and YOLOv7's own
`requirements.txt`) happen inside each version's own setup cell, not here.

**Expected output:** pip install logs.
...
**Expected output:** Drive mount confirmation.
from google.colab import drive
drive.mount('/content/drive', force_remount=False)
print("Drive mounted.")

## CELL 3 -- Define configuration and check GPU

Every common setting used across all 11 versions, defined once. Also checks GPU
availability -- training will be extremely slow or impractical without one, so
this is surfaced clearly up front rather than discovered mid-training.

**Expected output:** the printed configuration, and GPU name/CUDA version (or a
clear warning if no GPU is attached).
RANDOM_SEED = 0
EPOCHS = 50
IMAGE_SIZE = 640
BATCH_SIZE = 16
...
NUMBER_OF_CLASSES = 4
CLASS_NAMES = ["inclusion", "patches", "pitted_surface", "scratches"]
...
## CELL 4 -- Inspect the NEU-DET dataset

Locates the train/val image and annotation folders by pattern-matching (not a
hardcoded guess), then counts files, checks for missing image/XML pairs, samples
image dimensions, and reports objects per class. **Raises an error and stops** if
the dataset root doesn't exist or the folder structure can't be found -- per your
explicit instruction not to silently ignore this.

**Expected output:** the four detected paths, file counts, any mismatches, sample
dimensions, and per-class object counts.
...
for f in train_images[:5]:
    with Image.open(os.path.join(TRAIN_IMAGES_DIR, f)) as im:
        print(f"  {f}: size={im.size} (W x H), mode={im.mode}")
...
print("\nObjects per class (train+val combined):")
for cls, count in sorted(objects_per_class.items()):
    print(f"  {cls:16s}: {count}")

## CELL 5 -- Parse Pascal VOC XML

**What XML means:** Extensible Markup Language -- a plain-text way to store
structured, nested data.

**What Pascal VOC XML means:** the annotation convention from the Pascal Visual
Object Classes challenge. One `.xml` file per image, storing image width/height
and a list of `<object>` entries (class name + bounding box).

**How bounding boxes are stored:** each `<object>` has a `<bndbox>` with four
values -- `xmin`, `ymin` (top-left corner) and `xmax`, `ymax` (bottom-right
corner), all in raw pixel coordinates from the image's top-left at (0,0).

**Expected output:** two fully parsed examples printed as text.
...
## CELL 6 -- Verify classes and annotations

Scans every XML file for the class names actually present, checks each box is
internally valid (`xmin < xmax`, `ymin < ymax`, within image bounds), and
**stops with a clear error** if the discovered classes don't match the required
four -- rather than silently proceeding with a wrong mapping.

**Expected output:** discovered class names, and a validity report for every
bounding box.
def discover_classes(annot_dir, xml_files):
    names = set()
    for f in xml_files:
        _, _, objs = parse_voc_xml(os.path.join(annot_dir, f))
        for o in objs:
...
all_classes = discover_classes(TRAIN_ANNOT_DIR, train_xmls) | discover_classes(VAL_ANNOT_DIR, val_xmls)
EXPECTED_CLASSES = set(CLASS_NAMES)
print("Classes discovered:", sorted(all_classes))
print("Expected classes  :", sorted(EXPECTED_CLASSES))

if all_classes != EXPECTED_CLASSES:
    raise ValueError(f"Discovered classes {sorted(all_classes)} do not match the required "
                      f"CLASS_NAMES {sorted(EXPECTED_CLASSES)} -- fix your XML files or "
                      f"CLASS_NAMES before continuing.")
...
## CELL 7 -- Convert Pascal VOC XML annotations to YOLO TXT format

Converts every box to `class_id center_x center_y width height`, normalized 0-1.
**The original XML files in Drive are never touched** -- this only reads them and
writes new files elsewhere.

**Expected output:** conversion counts per split.
...
def convert_split(images_dir, annot_dir, image_files, split):
    ...
    for img_file in image_files:
        ...
        lines = []
        for o in objs:
            if o["name"] not in CLASS_MAP:
                print(f"  SKIPPING unknown class '{o['name']}' in {xml_path}"); continue
            cls_id = CLASS_MAP[o["name"]]
...
## CELL 8 -- Create the standardized dataset (used by ALL 11 models)

Builds ONE `NEU_YOLO/images/{train,val}` + `labels/{train,val}` + `data.yaml`
under `Dataset_Prepared/` on Drive. **Every one of the 11 versions trains from
this exact same copy** -- this is what makes the comparison fair; no model gets
its own slightly-different data pipeline.

**Expected output:** final file counts, and the printed `data.yaml`.
...
## CELL 9 -- Visualize the standardized dataset

Draws boxes decoded from the final YOLO `.txt` files onto sample images, trying
to show all four classes, so you can visually confirm the conversion is correct
before any model trains on it.

**Expected output:** a grid of images with boxes, class names, and class IDs.
...
## CELL 9B -- Shared reusable functions for all 11 YOLO versions

Rather than writing near-identical training/evaluation code 11 times, this cell
defines the reusable building blocks every version's cells call into --
`darknet_*` functions (v1, v2, v4), `ultralytics_*` functions (v3, v5, v8, v9,
v10, v11), and `yolov6_*` / `yolov7_*` functions. Each version's own cells are
then short, since they just call these with different parameters.

**mAP definitions used everywhere below, to be explicit:** mAP@0.5 uses IoU
threshold 0.5 (not 0.05 -- worth stating plainly since that's an easy typo to
make and a very different, much easier metric). mAP@0.5:0.95 averages across ten
thresholds from 0.50 to 0.95 in steps of 0.05.

**Expected output:** a confirmation that all shared functions loaded.
...
RESULT_COLUMNS = [
    ...
    "Best_Epoch", "AP_inclusion", "AP_patches", "AP_pitted_surface", "AP_scratches",
    "Status", "Notes",
]
...
def ultralytics_evaluate(model_name, model, pt_name, training_time_s):
    ...
    overall = {
        ...
        "AP_inclusion": ap_for("inclusion"), "AP_patches": ap_for("patches"),
...
def darknet_evaluate(darknet_bin, obj_data_path, cfg_path, backup_dir, training_time_s):
    ...
    overall = {
        ...
        "Model_Size_MB": model_size_mb, "Training_Time_s": training_time_s,
        "AP_inclusion": per_class.get("inclusion"), "AP_patches": per_class.get("patches"),
...
## CELL 60 -- YOLOv11 setup

**Architecture:** Anchor-free, decoupled head; C3k2 backbone blocks plus C2PSA partial self-attention
**Backbone:** C3k2 + C2PSA
**Framework/library:** PyTorch, via the `ultralytics` package
**Pretrained weights:** `yolo11n.pt` (COCO-pretrained, auto-downloaded on first use)


**Expected output:** GPU/model confirmation.
from ultralytics import YOLO
...
## CELL 61 -- YOLOv11 training

**Input image size:** 640, **Batch size:** 16, **Epochs:** 50
**Optimizer:** Ultralytics' default (SGD or its auto-selected optimizer depending
on version -- reported directly from the trainer args below rather than assumed)
**Learning rate:** Ultralytics' default schedule (reported from trainer args)
**Data augmentation:** Ultralytics' default augmentation pipeline (Mosaic,
HSV jitter, flips, etc.)

**Expected output:** an Ultralytics training progress table for 50 epochs.
...
## CELL 62 -- YOLOv11 evaluation

Evaluated on the same validation split used by every other version.

## Why not classification accuracy
Accuracy ignores whether a box was correctly placed or whether a defect was
missed -- precision, recall, and mAP are used because they capture both.

**Expected output:** Ultralytics' metrics summary table.
...
## CELL 63 -- YOLOv11 metrics and training curves

Reads Ultralytics' own `results.csv` (written every epoch during training) for
loss/precision/recall/mAP curves, and copies the confusion matrix + PR curve that
`model.val()` already generated in Cell 62 -- nothing here is regenerated by
hand.

**Expected output:** training curves and evaluation plots, saved under
`graphs/`.
ultralytics_curves("YOLOv11", project_dir_YOLOv11)
ultralytics_confusion_pr("YOLOv11", YOLOv11_metrics)

## CELL 64 -- YOLOv11 image inference

Upload an image and test the trained YOLOv11 model on it.

**Expected output:** the uploaded image with predicted boxes, class names, and
confidence scores, saved under `inference_results/`.
